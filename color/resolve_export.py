...
Timeline / working space is ACEScct (ACES workflow). Scene-linear
interchange is ACES2065-1. The graph is three serial nodes:
  ...
  2. WB   — scene-linear Bradford/CAT02 in ACEScg (AP1), wrapped in ACEScct.
            Bypass this node in Resolve to restore IDT → ACEScct →
...
DWG Intermediate helpers remain as an optional named export space only.
...
def wb_in_acescct(
    ...
    """WB node on an ACEScct timeline: decode → linear CAT in AP1 → encode."""
    ap1 = acescct_decode(np.asarray(acescct_rgb, dtype=np.float64))
    ap1 = apply_white_balance(
        ap1, cct, tint=tint, rgb_space=DEFAULT_WORKING_LINEAR, method=method
    )
    return _acescct_encode_lut(ap1)
...
def wb_cube_bytes(
    ...
    return format_cube(
        f"LogBridge WB Bradford CAT {cct:.0f}K tint {tint} (ACEScct in/out)",
        out,
        size,
    )
...
<LogBridgeResolveGraph version="1" status="implemented (unverified)">
  <WorkingSpace gamut="ACEScg" encoding="ACEScct" white="ACES" scene_linear="ACES2065-1"/>
  <Node index="1" name="IDT" type="LUT_or_CST" bypassable="false">
    <Description>Camera log to ACEScct via ACES2065-1. No white balance. ACES workflow.</Description>
{idt_block}
  </Node>
  <Node index="2" name="WB" type="Corrector" bypassable="true" enabled="{wb_enabled}" method="{_xml_escape(method)}">
    <Description>Scene-linear Bradford/CAT02 (CCT + tint) in ACEScg. Bypass this node in Resolve (Color page: disable node 2, or DCTL Bypass WB, or skip 02_WB.cube). Remaining graph is IDT → ACEScct → optional Rec.709 ODT.</Description>
    <CCT>{cct:.4f}</CCT>
    <Tint>{tint:.6f}</Tint>
  ...
  <Node index="3" name="ODT_Rec709" type="LUT_or_CST" bypassable="true" enabled="{odt_on}">
    <Description>Optional Rec.709 ODT. Not the only deliverable; timeline stays ACEScct when this node is off.</Description>
...
def export_resolve_bundle(
    ...
    *,
    ...
    Bypass flags come from ``graph`` when given (node 2 off = no WB bake).
    Default timeline is ACEScct / ACES2065-1, not DWG Intermediate.
    """
    dest = Path(dest)
