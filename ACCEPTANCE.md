...
- WB inspector shows three states: 机内 / 白平衡（估计） / 灰卡. Estimate chip lights only after confirm. Grey-card overrides.
...
- Right inspector: thinner Exposure + WB three states only (机内 / 白平衡（估计） / 灰卡). Estimate chip lights only after confirm.
