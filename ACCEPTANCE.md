...
- **处理已锁定片段** / **Apply graph** write one **ACES2065-1 AP0 proxy EXR sequence** (`{stem}_ACES2065-1_proxy/frame_000000.exr`) per locked clip. **整段代理，不是全精度成片.** Movie write walks `AVAssetReader` `copyNextSampleBuffer` forward (no per-frame seek / `AVAssetImageGenerator` / `copyCGImage` / `requestedTime`). Movie write decode uses source 10-bit / native-depth Y′CbCr → float (matrix-only; matrix and full/video from nclc/colr/vui; missing tags fail 「无法读取片源 Y′CbCr 矩阵/范围，未写出」; no silent 709-video default; video-range 10-bit is 64/876 not /1023; not the preview 8-bit path promoted to float). 8-bit Y′CbCr is only the fallback. Still a proxy, not camera-original. Not ACEScct. Not a Rec.709 .mov/.mp4. Pending / unlocked stay in the list with **先选择 Log 与色域** or **先选择成对 IDT** and produce no folder. Never guessed.
