...
def test_readme_bypass_wb_swift_zh():
    """README 51 验法: Swift 旁路白平衡短句. Py 长句英文冻另刀. ㉗–㊿ frozen. 邻行 Apply 冻. TITLE / XML / 色管冻. No alg."""
    from color.resolve_export import (
        EXPORT_NOTE_IN_CAMERA,
        GRAPH_DOT_CLIP_LABEL,
        GRAPH_DOT_EXP_FILE,
    ...
    assert py_src_bypass == py_bypass_frozen
    assert bypass_to not in py_apply
    assert bypass_to not in py_readme
    assert bypass_to != py_bypass_frozen
    ...
    assert generated_bypass == py_bypass_frozen
