...
def _apply_wb_line(text: str) -> str:
    for ln in text.splitlines():
        stripped = ln.strip()
...
def _apply_idt_line(text: str) -> str:
    for ln in text.splitlines():
        stripped = ln.strip()
        if stripped.startswith("- 应用 **IDT**") or stripped.startswith("- Apply **IDT**"):
            return stripped
    raise AssertionError("Apply IDT line missing")


def _apply_exposure_line(text: str) -> str:
    for ln in text.splitlines():
        stripped = ln.strip()
        if stripped.startswith("- 应用 **曝光**") or stripped.startswith("- Apply **Exposure**"):
            return stripped
...
def _bypass_exposure_wb_py_line(text: str) -> str:
    for ln in text.splitlines():
        stripped = ln.strip()
        if stripped.startswith("旁路曝光：") or stripped.startswith("To bypass Exposure:"):
            return stripped
    raise AssertionError("Py bypass Exposure/WB line missing")


def _deliverable_warn_py_line(text: str) -> str:
    for ln in text.splitlines():
        stripped = ln.strip()
        if stripped.startswith("不要把单个 Rec.709") or stripped.startswith(
            "Do not use a single Rec.709"
        ):
            return stripped
    raise AssertionError("Py Rec.709 deliverable warning missing")
...
def _files_wb_cube_row(text: str) -> str:
    for ln in text.splitlines():
...
def _files_wb_cdl_row(text: str) -> str:
    for ln in text.splitlines():
...
def _files_wb_dctl_row(text: str) -> str:
    for ln in text.splitlines():
...
def _files_odt_row(text: str) -> str:
    for ln in text.splitlines():
...
README_BYPASS_WB_PY_TO = (
    "旁路曝光：关掉节点 2（或勾 DCTL **Bypass Exposure**，或停在 0 档）。"
    "旁路白平衡：关掉节点 3（或勾 DCTL **Bypass WB**，或跳过 CDL/LUT）。"
    "剩余图：**IDT →（可选曝光）→ ACEScct → 可选 Rec.709 ODT**。"
)
README_BYPASS_WB_PY_FROM = (
...
README_BYPASS_WB_PY_BANNED = (
    "To bypass Exposure",
    "The remaining graph is",
    "剩余图是",
    "档位留 0",
)
README_DELIVERABLE_WARN_PY_TO = (
    "不要把单个 Rec.709 文件当作唯一交付物。Rec.709 仅预览。"
)
README_DELIVERABLE_WARN_PY_FROM = (
    "Do not use a single Rec.709 file as the only deliverable. Rec.709 is preview only."
)
README_DELIVERABLE_WARN_PY_BANNED = (
    "Do not use a single Rec.709",
    "as the only deliverable",
...
README_FILES_WB_CUBE_TO = "白平衡查找表（Bradford CAT，ACEScct 封装）"
README_FILES_WB_CUBE_FROM = "WB LUT (Bradford CAT, ACEScct-wrapped)"
README_FILES_WB_CUBE_BANNED = "WB LUT"
README_FILES_ROW_WB_CUBE = (
    "| `03_WB.cube` | 白平衡查找表（Bradford CAT，ACEScct 封装） |"
)
README_FILES_WB_CDL_TO = "白平衡 ASC CDL 校色器"
README_FILES_WB_CDL_FROM = "WB as ASC CDL Color Corrector"
README_FILES_WB_CDL_BANNED = "WB as ASC CDL Color Corrector"
README_FILES_ROW_WB_CDL = (
    "| `03_WB.cdl` / `03_WB.ccc` | 白平衡 ASC CDL 校色器 |"
)
README_FILES_WB_DCTL_TO = "白平衡 DCTL（精确 3×3）"
README_FILES_WB_DCTL_FROM = "WB as DCTL (exact 3×3)"
README_FILES_WB_DCTL_BANNED = "WB as DCTL (exact 3×3)"
README_FILES_ROW_WB_DCTL = "| `03_WB.dctl` | 白平衡 DCTL（精确 3×3） |"
README_FILES_ODT_TO = "709 预览（BT.709 OETF，不是 ACES OT）"
README_FILES_ODT_FROM = "709 预览 (BT.709 OETF, not ACES OT)"
README_FILES_ODT_BANNED = "not ACES OT"
...
README_TRAIL_CLAIM_TO = "任何精度声明前须有灰卡样张。"
README_TRAIL_CLAIM_FROM = "Golden grey-card samples are required before any accuracy claim."
README_TRAIL_CLAIM_BANNED = "Golden grey-card samples"
README_TRAIL_FOLLOWING = "已实现（未验证）。"
README_TRAIL_FOLLOWING_FROM = "Implemented (unverified)."
README_TRAIL_FOLLOWING_BANNED = "Implemented (unverified)."
...
def test_readme_apply_odt_plain_chinese(tmp_path: Path):
    ...
    # Apply 其余冻：IDT 见㊽. Exposure 见㊿. WB 行见㊼.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert "- 应用 **白平衡**" in swift_apply
    assert "- 应用 **白平衡**" in py_apply
    assert "- 应用 **曝光**" in py_apply
    assert (
        "- 应用 **IDT**（节点 1：LUT `01_IDT_*.cube`，或 ACES IDT / CST 相机 → ACEScct）。"
        in swift_apply
    )
...
def test_readme_files_wb_cube_plain_chinese(tmp_path: Path):
    """README Files ㊴: 03_WB Role 人话. Swift↔Py 该格一致. ㉗–㊳ + TITLE / XML / 色管 / Files 其余行 frozen."""
    assert README_FILES_WB_CUBE_TO == "白平衡查找表（Bradford CAT，ACEScct 封装）"
    assert README_FILES_WB_CUBE_FROM == "WB LUT (Bradford CAT, ACEScct-wrapped)"
    assert README_FILES_WB_CUBE_BANNED == "WB LUT"
    assert README_FILES_ROW_WB_CUBE == (
        "| `03_WB.cube` | 白平衡查找表（Bradford CAT，ACEScct 封装） |"
    )
    assert README_FILES_WB_CUBE_TO in README_FILES_ROW_WB_CUBE
    assert README_FILES_WB_CUBE_FROM not in README_FILES_ROW_WB_CUBE
    assert README_FILES_WB_CUBE_BANNED in README_FILES_WB_CUBE_FROM
    assert README_FILES_WB_CUBE_BANNED not in README_FILES_WB_CUBE_TO
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    wb_row = _files_wb_cube_row(readme)

    assert wb_row == README_FILES_ROW_WB_CUBE
    assert README_FILES_WB_CUBE_TO in wb_row
    assert README_FILES_WB_CUBE_FROM not in wb_row
    assert README_FILES_WB_CUBE_FROM not in files
    assert README_FILES_WB_CUBE_FROM not in readme
    assert README_FILES_WB_CUBE_BANNED not in wb_row
    assert README_FILES_WB_CUBE_BANNED not in files
    assert README_FILES_WB_CUBE_BANNED not in readme

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_files = _files_section(generated)
    generated_row = _files_wb_cube_row(generated)
    assert generated_row == wb_row
    assert generated_row == README_FILES_ROW_WB_CUBE
    assert README_FILES_WB_CUBE_FROM not in generated
    assert README_FILES_WB_CUBE_FROM not in generated_files
    assert README_FILES_WB_CUBE_BANNED not in generated
    assert README_FILES_WB_CUBE_BANNED not in generated_files
    ...
    swift_files = _files_section(readme_fn)
    py_files = _files_section(py_readme)
    swift_wb = _files_wb_cube_row(readme_fn)
    py_wb_row = _files_wb_cube_row(py_readme)
    swift_idt = _files_idt_row(readme_fn)
    py_idt_row = _files_idt_row(py_readme)
    swift_xml = _files_graph_xml_row(readme_fn)
    py_xml_row = _files_graph_xml_row(py_readme)
    ...
    # 验法㊴-1: one TO 一字不差. Swift↔Py 该格一致；Files 其余冻.
    assert swift_wb == README_FILES_ROW_WB_CUBE
    assert py_wb_row == README_FILES_ROW_WB_CUBE
    assert swift_wb == py_wb_row
    assert README_FILES_WB_CUBE_TO in swift_wb
    assert README_FILES_WB_CUBE_TO in py_wb_row
    assert README_FILES_WB_CUBE_FROM not in swift_wb
    assert README_FILES_WB_CUBE_FROM not in py_wb_row
    assert README_FILES_WB_CUBE_FROM not in swift_files
    assert README_FILES_WB_CUBE_FROM not in py_files
    assert README_FILES_WB_CUBE_FROM not in readme_fn
    assert README_FILES_WB_CUBE_FROM not in py_readme
    assert README_FILES_WB_CUBE_FROM not in swift
    assert README_FILES_WB_CUBE_FROM not in py
    assert README_FILES_WB_CUBE_BANNED not in swift_wb
    assert README_FILES_WB_CUBE_BANNED not in py_wb_row
    assert README_FILES_WB_CUBE_BANNED not in swift_files
    assert README_FILES_WB_CUBE_BANNED not in py_files
    assert README_FILES_WB_CUBE_BANNED not in readme_fn
    assert README_FILES_WB_CUBE_BANNED not in py_readme
    assert README_FILES_WB_CUBE_BANNED not in swift
    assert README_FILES_WB_CUBE_BANNED not in py

    # Files 其余行冻（含 01_IDT / graph.xml Swift/Py 分锁；不借此刀对齐曝光格）.
    assert swift_idt == README_FILES_ROW_IDT_SWIFT
    assert py_idt_row == README_FILES_ROW_IDT_PY
    assert swift_idt != py_idt_row
    assert README_FILES_ROW_IDT_SWIFT != README_FILES_ROW_IDT_PY
    ...
    # ㉗–㊳ locked strings 一字不动（含 01_IDT Role 分锁 / 本说明 / graph.xml 机器可读分锁 / | 文件 | 作用 | / 调色页，串行节点图： / 同一图的 Graphviz）.
    assert README_FILES_IDT_SWIFT_TO == "IDT 查找表（不含白平衡）"
    assert README_FILES_IDT_PY_TO == "IDT 查找表（不含白平衡、不含曝光）"
    assert README_FILES_ROW_IDT_SWIFT == (
        "| `01_IDT_<idt>.cube` | IDT 查找表（不含白平衡） |"
    ...
    assert _files_idt_row(readme) == README_FILES_ROW_IDT_PY
    assert _files_idt_row(readme_fn) == README_FILES_ROW_IDT_SWIFT
    assert _files_idt_row(py_readme) == README_FILES_ROW_IDT_PY
    assert README_FILES_IDT_SWIFT_FROM not in swift_files
    assert README_FILES_IDT_PY_FROM not in py_files
    assert README_FILES_IDT_BANNED not in readme_fn
    assert README_FILES_IDT_BANNED not in py_readme
    assert README_FILES_README_TO == "本说明"
    assert _files_readme_row(readme) == README_FILES_ROW_README
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in swift_wb
    assert "达芬奇已验证" not in py_wb_row
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(swift_wb)
    _assert_chengpian_not_a_deliverable_claim(py_wb_row)
    _assert_chengpian_not_a_deliverable_claim(README_FILES_WB_CUBE_TO)


def test_readme_files_wb_cdl_plain_chinese(tmp_path: Path):
    """README Files ㊵: 03_WB.cdl / 03_WB.ccc Role 人话. Swift↔Py 该格一致. ㉗–㊴ + TITLE / XML / 色管 / Files 其余行 frozen."""
    assert README_FILES_WB_CDL_TO == "白平衡 ASC CDL 校色器"
    assert README_FILES_WB_CDL_FROM == "WB as ASC CDL Color Corrector"
    assert README_FILES_WB_CDL_BANNED == "WB as ASC CDL Color Corrector"
    assert README_FILES_ROW_WB_CDL == (
        "| `03_WB.cdl` / `03_WB.ccc` | 白平衡 ASC CDL 校色器 |"
    )
    assert README_FILES_WB_CDL_TO in README_FILES_ROW_WB_CDL
    assert README_FILES_WB_CDL_FROM not in README_FILES_ROW_WB_CDL
    assert README_FILES_WB_CDL_BANNED in README_FILES_WB_CDL_FROM
    assert README_FILES_WB_CDL_BANNED not in README_FILES_WB_CDL_TO
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    cdl_row = _files_wb_cdl_row(readme)

    assert cdl_row == README_FILES_ROW_WB_CDL
    assert README_FILES_WB_CDL_TO in cdl_row
    assert README_FILES_WB_CDL_FROM not in cdl_row
    assert README_FILES_WB_CDL_FROM not in files
    assert README_FILES_WB_CDL_FROM not in readme
    assert README_FILES_WB_CDL_BANNED not in cdl_row
    assert README_FILES_WB_CDL_BANNED not in files
    assert README_FILES_WB_CDL_BANNED not in readme

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_files = _files_section(generated)
    generated_row = _files_wb_cdl_row(generated)
    assert generated_row == cdl_row
    assert generated_row == README_FILES_ROW_WB_CDL
    assert README_FILES_WB_CDL_FROM not in generated
    assert README_FILES_WB_CDL_FROM not in generated_files
    assert README_FILES_WB_CDL_BANNED not in generated
    assert README_FILES_WB_CDL_BANNED not in generated_files
    ...
    swift_files = _files_section(readme_fn)
    py_files = _files_section(py_readme)
    swift_cdl = _files_wb_cdl_row(readme_fn)
    py_cdl_row = _files_wb_cdl_row(py_readme)
    swift_wb = _files_wb_cube_row(readme_fn)
    py_wb_row = _files_wb_cube_row(py_readme)
    swift_idt = _files_idt_row(readme_fn)
    py_idt_row = _files_idt_row(py_readme)
    ...
    # 验法㊵-1: one TO 一字不差. Swift↔Py 该格一致；Files 其余冻.
    assert swift_cdl == README_FILES_ROW_WB_CDL
    assert py_cdl_row == README_FILES_ROW_WB_CDL
    assert swift_cdl == py_cdl_row
    assert README_FILES_WB_CDL_TO in swift_cdl
    assert README_FILES_WB_CDL_TO in py_cdl_row
    assert README_FILES_WB_CDL_FROM not in swift_cdl
    assert README_FILES_WB_CDL_FROM not in py_cdl_row
    assert README_FILES_WB_CDL_FROM not in swift_files
    assert README_FILES_WB_CDL_FROM not in py_files
    assert README_FILES_WB_CDL_FROM not in readme_fn
    assert README_FILES_WB_CDL_FROM not in py_readme
    assert README_FILES_WB_CDL_FROM not in swift
    assert README_FILES_WB_CDL_FROM not in py
    assert README_FILES_WB_CDL_BANNED not in swift_cdl
    assert README_FILES_WB_CDL_BANNED not in py_cdl_row
    assert README_FILES_WB_CDL_BANNED not in swift_files
    assert README_FILES_WB_CDL_BANNED not in py_files
    assert README_FILES_WB_CDL_BANNED not in readme_fn
    assert README_FILES_WB_CDL_BANNED not in py_readme
    assert README_FILES_WB_CDL_BANNED not in swift
    assert README_FILES_WB_CDL_BANNED not in py

    # Files 其余行冻（含 03_WB.cube ㊴ / 01_IDT / graph.xml Swift/Py 分锁；dctl Role 冻；不借此刀对齐曝光格）.
    assert swift_wb == README_FILES_ROW_WB_CUBE
    assert py_wb_row == README_FILES_ROW_WB_CUBE
    assert swift_wb == py_wb_row
    assert README_FILES_WB_CUBE_TO == "白平衡查找表（Bradford CAT，ACEScct 封装）"
    ...
    assert README_FILES_ROW_WB_CUBE in swift_files
    assert README_FILES_ROW_WB_CUBE in py_files
    assert README_FILES_ROW_WB_DCTL == "| `03_WB.dctl` | 白平衡 DCTL（精确 3×3） |"
    assert README_FILES_ROW_WB_DCTL in swift_files
    assert README_FILES_ROW_WB_DCTL in py_files
    assert README_FILES_ROW_ODT in swift_files
    assert README_FILES_ROW_ODT in py_files
    ...
    assert README_FILES_ROW_EXP_CUBE in files
    assert README_FILES_ROW_WB_CUBE in files
    assert README_FILES_ROW_WB_CDL in files
    assert README_FILES_ROW_WB_DCTL in files
    assert README_FILES_ROW_README in files

    # ㉗–㊴ locked strings 一字不动（含 白平衡查找表（Bradford CAT，ACEScct 封装） / 01_IDT Role 分锁 / 本说明 / graph.xml 机器可读分锁 / | 文件 | 作用 | / 调色页，串行节点图： / 同一图的 Graphviz）.
    assert README_FILES_WB_CUBE_TO == "白平衡查找表（Bradford CAT，ACEScct 封装）"
    assert README_FILES_ROW_WB_CUBE == (
        "| `03_WB.cube` | 白平衡查找表（Bradford CAT，ACEScct 封装） |"
    )
    assert _files_wb_cube_row(readme) == README_FILES_ROW_WB_CUBE
    assert _files_wb_cube_row(readme_fn) == README_FILES_ROW_WB_CUBE
    assert _files_wb_cube_row(py_readme) == README_FILES_ROW_WB_CUBE
    assert README_FILES_WB_CUBE_FROM not in swift_files
    assert README_FILES_WB_CUBE_FROM not in py_files
    assert README_FILES_WB_CUBE_BANNED not in readme_fn
    assert README_FILES_WB_CUBE_BANNED not in py_readme
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in swift_cdl
    assert "达芬奇已验证" not in py_cdl_row
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(swift_cdl)
    _assert_chengpian_not_a_deliverable_claim(py_cdl_row)
    _assert_chengpian_not_a_deliverable_claim(README_FILES_WB_CDL_TO)


def test_readme_files_wb_dctl_plain_chinese(tmp_path: Path):
    """README Files ㊶: 03_WB.dctl Role 人话. Swift↔Py 该格一致. ㉗–㊵ + TITLE / XML / 色管 / Files 其余行 frozen."""
    assert README_FILES_WB_DCTL_TO == "白平衡 DCTL（精确 3×3）"
    assert README_FILES_WB_DCTL_FROM == "WB as DCTL (exact 3×3)"
    assert README_FILES_WB_DCTL_BANNED == "WB as DCTL (exact 3×3)"
    assert README_FILES_ROW_WB_DCTL == "| `03_WB.dctl` | 白平衡 DCTL（精确 3×3） |"
    assert README_FILES_WB_DCTL_TO in README_FILES_ROW_WB_DCTL
    assert README_FILES_WB_DCTL_FROM not in README_FILES_ROW_WB_DCTL
    assert README_FILES_WB_DCTL_BANNED in README_FILES_WB_DCTL_FROM
    assert README_FILES_WB_DCTL_BANNED not in README_FILES_WB_DCTL_TO
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    dctl_row = _files_wb_dctl_row(readme)

    assert dctl_row == README_FILES_ROW_WB_DCTL
    assert README_FILES_WB_DCTL_TO in dctl_row
    assert README_FILES_WB_DCTL_FROM not in dctl_row
    assert README_FILES_WB_DCTL_FROM not in files
    assert README_FILES_WB_DCTL_FROM not in readme
    assert README_FILES_WB_DCTL_BANNED not in dctl_row
    assert README_FILES_WB_DCTL_BANNED not in files
    assert README_FILES_WB_DCTL_BANNED not in readme

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_files = _files_section(generated)
    generated_row = _files_wb_dctl_row(generated)
    assert generated_row == dctl_row
    assert generated_row == README_FILES_ROW_WB_DCTL
    assert README_FILES_WB_DCTL_FROM not in generated
    assert README_FILES_WB_DCTL_FROM not in generated_files
    assert README_FILES_WB_DCTL_BANNED not in generated
    assert README_FILES_WB_DCTL_BANNED not in generated_files
    ...
    swift_files = _files_section(readme_fn)
    py_files = _files_section(py_readme)
    swift_dctl = _files_wb_dctl_row(readme_fn)
    py_dctl_row = _files_wb_dctl_row(py_readme)
    swift_cdl = _files_wb_cdl_row(readme_fn)
    py_cdl_row = _files_wb_cdl_row(py_readme)
    swift_wb = _files_wb_cube_row(readme_fn)
    py_wb_row = _files_wb_cube_row(py_readme)
    ...
    # 验法㊶-1: one TO 一字不差. Swift↔Py 该格一致；Files 其余冻.
    assert swift_dctl == README_FILES_ROW_WB_DCTL
    assert py_dctl_row == README_FILES_ROW_WB_DCTL
    assert swift_dctl == py_dctl_row
    assert README_FILES_WB_DCTL_TO in swift_dctl
    assert README_FILES_WB_DCTL_TO in py_dctl_row
    assert README_FILES_WB_DCTL_FROM not in swift_dctl
    assert README_FILES_WB_DCTL_FROM not in py_dctl_row
    assert README_FILES_WB_DCTL_FROM not in swift_files
    assert README_FILES_WB_DCTL_FROM not in py_files
    assert README_FILES_WB_DCTL_FROM not in readme_fn
    assert README_FILES_WB_DCTL_FROM not in py_readme
    assert README_FILES_WB_DCTL_FROM not in swift
    assert README_FILES_WB_DCTL_FROM not in py
    assert README_FILES_WB_DCTL_BANNED not in swift_dctl
    assert README_FILES_WB_DCTL_BANNED not in py_dctl_row
    assert README_FILES_WB_DCTL_BANNED not in swift_files
    assert README_FILES_WB_DCTL_BANNED not in py_files
    assert README_FILES_WB_DCTL_BANNED not in readme_fn
    assert README_FILES_WB_DCTL_BANNED not in py_readme
    assert README_FILES_WB_DCTL_BANNED not in swift
    assert README_FILES_WB_DCTL_BANNED not in py

    # Files 其余行冻（含 03_WB.cdl ㊵ / 03_WB.cube ㊴ / 01_IDT / graph.xml Swift/Py 分锁；不借此刀对齐曝光格）.
    assert swift_cdl == README_FILES_ROW_WB_CDL
    assert py_cdl_row == README_FILES_ROW_WB_CDL
    assert swift_cdl == py_cdl_row
    assert README_FILES_WB_CDL_TO == "白平衡 ASC CDL 校色器"
    ...
    # ㉗–㊵ locked strings 一字不动（含 白平衡 ASC CDL 校色器 / 白平衡查找表（Bradford CAT，ACEScct 封装） / 01_IDT Role 分锁 / 本说明 / graph.xml 机器可读分锁 / | 文件 | 作用 | / 调色页，串行节点图： / 同一图的 Graphviz）.
    assert README_FILES_WB_CDL_TO == "白平衡 ASC CDL 校色器"
    assert README_FILES_ROW_WB_CDL == (
        "| `03_WB.cdl` / `03_WB.ccc` | 白平衡 ASC CDL 校色器 |"
    )
    assert _files_wb_cdl_row(readme) == README_FILES_ROW_WB_CDL
    assert _files_wb_cdl_row(readme_fn) == README_FILES_ROW_WB_CDL
    assert _files_wb_cdl_row(py_readme) == README_FILES_ROW_WB_CDL
    assert README_FILES_WB_CDL_FROM not in swift_files
    assert README_FILES_WB_CDL_FROM not in py_files
    assert README_FILES_WB_CDL_BANNED not in readme_fn
    assert README_FILES_WB_CDL_BANNED not in py_readme
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in swift_dctl
    assert "达芬奇已验证" not in py_dctl_row
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(swift_dctl)
    _assert_chengpian_not_a_deliverable_claim(py_dctl_row)
    _assert_chengpian_not_a_deliverable_claim(README_FILES_WB_DCTL_TO)


def test_readme_files_odt_plain_chinese(tmp_path: Path):
    """README Files ㊷: 04_ODT_Rec709.cube Role 人话. Swift↔Py 该格一致. ㉗–㊶ + TITLE / XML / 色管 / Files 其余行 frozen."""
    assert README_FILES_ODT_TO == "709 预览（BT.709 OETF，不是 ACES OT）"
    assert README_FILES_ODT_FROM == "709 预览 (BT.709 OETF, not ACES OT)"
    assert README_FILES_ODT_BANNED == "not ACES OT"
    assert README_FILES_ROW_ODT == (
        "| `04_ODT_Rec709.cube` | 709 预览（BT.709 OETF，不是 ACES OT） |"
    )
    assert README_FILES_ODT_TO in README_FILES_ROW_ODT
    assert README_FILES_ODT_FROM not in README_FILES_ROW_ODT
    assert README_FILES_ODT_BANNED not in README_FILES_ROW_ODT
    assert README_FILES_ODT_BANNED in README_FILES_ODT_FROM
    assert README_FILES_ODT_BANNED not in README_FILES_ODT_TO
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    odt_row = _files_odt_row(readme)

    assert odt_row == README_FILES_ROW_ODT
    assert README_FILES_ODT_TO in odt_row
    assert README_FILES_ODT_FROM not in odt_row
    assert README_FILES_ODT_FROM not in files
    assert README_FILES_ODT_FROM not in readme
    assert README_FILES_ODT_BANNED not in odt_row
    assert README_FILES_ODT_BANNED not in files

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_files = _files_section(generated)
    generated_row = _files_odt_row(generated)
    assert generated_row == odt_row
    assert generated_row == README_FILES_ROW_ODT
    assert README_FILES_ODT_FROM not in generated
    assert README_FILES_ODT_FROM not in generated_files
    assert README_FILES_ODT_BANNED not in generated_row
    assert README_FILES_ODT_BANNED not in generated_files
    ...
    swift_files = _files_section(readme_fn)
    py_files = _files_section(py_readme)
    swift_odt = _files_odt_row(readme_fn)
    py_odt_row = _files_odt_row(py_readme)
    swift_dctl = _files_wb_dctl_row(readme_fn)
    py_dctl_row = _files_wb_dctl_row(py_readme)
    swift_cdl = _files_wb_cdl_row(readme_fn)
    py_cdl_row = _files_wb_cdl_row(py_readme)
    ...
    # 验法㊷-1: one TO 一字不差. Swift↔Py 该格一致；Files 其余冻.
    assert swift_odt == README_FILES_ROW_ODT
    assert py_odt_row == README_FILES_ROW_ODT
    assert swift_odt == py_odt_row
    assert README_FILES_ODT_TO in swift_odt
    assert README_FILES_ODT_TO in py_odt_row
    assert README_FILES_ODT_FROM not in swift_odt
    assert README_FILES_ODT_FROM not in py_odt_row
    assert README_FILES_ODT_FROM not in swift_files
    assert README_FILES_ODT_FROM not in py_files
    assert README_FILES_ODT_FROM not in readme_fn
    assert README_FILES_ODT_FROM not in py_readme
    assert README_FILES_ODT_FROM not in swift
    assert README_FILES_ODT_FROM not in py
    assert README_FILES_ODT_BANNED not in swift_odt
    assert README_FILES_ODT_BANNED not in py_odt_row
    assert README_FILES_ODT_BANNED not in swift_files
    assert README_FILES_ODT_BANNED not in py_files

    # Files 其余行冻（含 03_WB.dctl ㊶ / 03_WB.cdl ㊵ / 03_WB.cube ㊴ / 01_IDT / graph.xml Swift/Py 分锁；不借此刀对齐曝光格）.
    assert swift_dctl == README_FILES_ROW_WB_DCTL
    assert py_dctl_row == README_FILES_ROW_WB_DCTL
    assert swift_dctl == py_dctl_row
    assert README_FILES_WB_DCTL_TO == "白平衡 DCTL（精确 3×3）"
    ...
    # ㉗–㊶ locked strings 一字不动（含 白平衡 DCTL（精确 3×3） / 白平衡 ASC CDL 校色器 / 白平衡查找表（Bradford CAT，ACEScct 封装） / 01_IDT Role 分锁 / 本说明 / graph.xml 机器可读分锁 / | 文件 | 作用 | / 调色页，串行节点图： / 同一图的 Graphviz）.
    assert README_FILES_WB_DCTL_TO == "白平衡 DCTL（精确 3×3）"
    assert README_FILES_ROW_WB_DCTL == "| `03_WB.dctl` | 白平衡 DCTL（精确 3×3） |"
    assert _files_wb_dctl_row(readme) == README_FILES_ROW_WB_DCTL
    assert _files_wb_dctl_row(readme_fn) == README_FILES_ROW_WB_DCTL
    assert _files_wb_dctl_row(py_readme) == README_FILES_ROW_WB_DCTL
    assert README_FILES_WB_DCTL_FROM not in swift_files
    assert README_FILES_WB_DCTL_FROM not in py_files
    assert README_FILES_WB_DCTL_BANNED not in readme_fn
    assert README_FILES_WB_DCTL_BANNED not in py_readme
    ...
    assert "达芬奇已验证" not in py_odt_row
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(swift_odt)
    _assert_chengpian_not_a_deliverable_claim(py_odt_row)
    _assert_chengpian_not_a_deliverable_claim(README_FILES_ODT_TO)


def test_readme_trail_half_plain_chinese(tmp_path: Path):
    """README ㊻: 半段 已实现（未验证）。 Swift↔Py 该半段一致. ㉗–㊺ + TITLE / XML / 色管 / Files 行 frozen."""
    ...
    assert README_TRAIL_CLAIM_TO == "任何精度声明前须有灰卡样张。"
    assert README_TRAIL_CLAIM_FROM == (
        "Golden grey-card samples are required before any accuracy claim."
    )
    assert README_TRAIL_CLAIM_BANNED == "Golden grey-card samples"
    assert README_TRAIL_FOLLOWING == "已实现（未验证）。"
    assert README_TRAIL_FOLLOWING_FROM == "Implemented (unverified)."
    assert README_TRAIL_FOLLOWING_BANNED == "Implemented (unverified)."
    ...
    assert README_TRAIL_BLOCK_FROM == (
        "M1 is a serial node graph (IDT → Exposure → WB → ODT), not a general node editor. "
        "Golden grey-card samples are required before any accuracy claim. "
        "Implemented (unverified)."
    )
    ...
    assert README_TRAIL_HALF_BANNED in README_TRAIL_SENTENCE_FROM
    assert README_TRAIL_HALF_BANNED in README_TRAIL_HALF_FROM
    assert README_TRAIL_HALF_BANNED not in README_TRAIL_HALF_TO
    assert README_TRAIL_CLAIM_TO in README_TRAIL_BLOCK_TO
    assert README_TRAIL_CLAIM_FROM not in README_TRAIL_BLOCK_TO
    assert README_TRAIL_CLAIM_BANNED not in README_TRAIL_BLOCK_TO
    assert README_TRAIL_CLAIM_BANNED in README_TRAIL_CLAIM_FROM
    assert README_TRAIL_CLAIM_BANNED not in README_TRAIL_CLAIM_TO
    assert README_TRAIL_BLOCK_TO.startswith(README_TRAIL_SENTENCE_TO)
    assert README_TRAIL_BLOCK_TO.endswith(README_TRAIL_FOLLOWING)
    assert README_TRAIL_FOLLOWING in README_TRAIL_BLOCK_TO
    assert README_TRAIL_FOLLOWING_FROM not in README_TRAIL_BLOCK_TO
    assert README_TRAIL_FOLLOWING_BANNED not in README_TRAIL_BLOCK_TO
    assert README_TRAIL_FOLLOWING_BANNED in README_TRAIL_FOLLOWING_FROM
    assert README_TRAIL_FOLLOWING_BANNED not in README_TRAIL_FOLLOWING
    assert README_TRAIL_FOLLOWING_FROM in README_TRAIL_BLOCK_FROM
    assert README_TRAIL_FOLLOWING_FROM in README_TRAIL_BLOCK_PRIOR
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    trail = _readme_trail_line(readme)
    ...
    assert README_TRAIL_FOLLOWING_FROM not in trail
    assert README_TRAIL_FOLLOWING_FROM not in files
    assert README_TRAIL_FOLLOWING_BANNED not in trail
    assert README_TRAIL_FOLLOWING_BANNED not in files
    ...
    assert README_TRAIL_HALF_BANNED not in trail
    assert README_TRAIL_HALF_BANNED not in files
    assert README_TRAIL_CLAIM_FROM not in trail
    assert README_TRAIL_CLAIM_FROM not in files
    assert README_TRAIL_CLAIM_FROM not in readme
    assert README_TRAIL_CLAIM_BANNED not in trail
    assert README_TRAIL_CLAIM_BANNED not in files
    assert README_TRAIL_CLAIM_BANNED not in readme
    assert README_TRAIL_BLOCK_FROM not in readme
    assert README_TRAIL_BLOCK_PRIOR not in readme
    assert README_TRAIL_SENTENCE_FROM not in readme

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_files = _files_section(generated)
    generated_trail = _readme_trail_line(generated)
    ...
    assert README_TRAIL_FOLLOWING_FROM not in generated_trail
    assert README_TRAIL_FOLLOWING_FROM not in generated_files
    assert README_TRAIL_FOLLOWING_BANNED not in generated_trail
    assert README_TRAIL_FOLLOWING_BANNED not in generated_files
    ...
    assert README_TRAIL_HALF_BANNED not in generated_files
    assert README_TRAIL_CLAIM_FROM not in generated
    assert README_TRAIL_CLAIM_FROM not in generated_files
    assert README_TRAIL_CLAIM_BANNED not in generated_trail
    assert README_TRAIL_CLAIM_BANNED not in generated_files
    assert README_TRAIL_CLAIM_BANNED not in generated
    assert README_TRAIL_BLOCK_FROM not in generated
    assert README_TRAIL_BLOCK_PRIOR not in generated
    ...
    swift_files = _files_section(readme_fn)
    py_files = _files_section(py_readme)
    swift_trail = _readme_trail_line(readme_fn)
    py_trail = _readme_trail_line(py_readme)
    swift_odt = _files_odt_row(readme_fn)
    py_odt_row = _files_odt_row(py_readme)
    swift_dctl = _files_wb_dctl_row(readme_fn)
    py_dctl_row = _files_wb_dctl_row(py_readme)
    ...
    # 验法㊻-1: 半段 TO 一字不差. ㊸ / ㊹ / ㊺ 半段冻. Swift↔Py 该半段一致. English Implemented 留在 XML/注释/HLG·PQ.
    ...
    assert README_TRAIL_CLAIM_TO in py_trail
    assert README_TRAIL_FOLLOWING in swift_trail
    assert README_TRAIL_FOLLOWING in py_trail
    assert README_TRAIL_FOLLOWING_FROM not in swift_trail
    assert README_TRAIL_FOLLOWING_FROM not in py_trail
    assert README_TRAIL_FOLLOWING_FROM not in swift_files
    assert README_TRAIL_FOLLOWING_FROM not in py_files
    assert README_TRAIL_FOLLOWING_BANNED not in swift_trail
    assert README_TRAIL_FOLLOWING_BANNED not in py_trail
    assert README_TRAIL_FOLLOWING_BANNED not in swift_files
    assert README_TRAIL_FOLLOWING_BANNED not in py_files
    ...
    assert README_TRAIL_HALF_FROM not in py_files
    assert README_TRAIL_HALF_FROM not in readme_fn
    assert README_TRAIL_HALF_FROM not in py_readme
    assert README_TRAIL_HALF_FROM not in swift
    assert README_TRAIL_HALF_FROM not in py
    assert README_TRAIL_HALF_BANNED not in swift_trail
    assert README_TRAIL_HALF_BANNED not in py_trail
    assert README_TRAIL_HALF_BANNED not in swift_files
    assert README_TRAIL_HALF_BANNED not in py_files
    assert README_TRAIL_HALF_BANNED not in readme_fn
    assert README_TRAIL_HALF_BANNED not in py_readme
    assert README_TRAIL_HALF_BANNED not in swift
    assert README_TRAIL_HALF_BANNED not in py
    assert README_TRAIL_CLAIM_FROM not in swift_trail
    assert README_TRAIL_CLAIM_FROM not in py_trail
    assert README_TRAIL_CLAIM_FROM not in swift_files
    assert README_TRAIL_CLAIM_FROM not in py_files
    assert README_TRAIL_CLAIM_FROM not in readme_fn
    assert README_TRAIL_CLAIM_FROM not in py_readme
    assert README_TRAIL_CLAIM_FROM not in swift
    assert README_TRAIL_CLAIM_FROM not in py
    assert README_TRAIL_CLAIM_BANNED not in swift_trail
    assert README_TRAIL_CLAIM_BANNED not in py_trail
    assert README_TRAIL_CLAIM_BANNED not in swift_files
    assert README_TRAIL_CLAIM_BANNED not in py_files
    assert README_TRAIL_CLAIM_BANNED not in readme_fn
    assert README_TRAIL_CLAIM_BANNED not in py_readme
    assert README_TRAIL_CLAIM_BANNED not in swift
    assert README_TRAIL_CLAIM_BANNED not in py
    assert README_TRAIL_SENTENCE_FROM not in swift
    assert README_TRAIL_SENTENCE_FROM not in py
    assert README_TRAIL_BLOCK_FROM not in swift
    assert README_TRAIL_BLOCK_FROM not in py
    assert README_TRAIL_BLOCK_PRIOR not in swift
    assert README_TRAIL_BLOCK_PRIOR not in py

    # Files 其余行冻（含 04_ODT ㊷ / 03_WB.dctl ㊶ / 03_WB.cdl ㊵ / 03_WB.cube ㊴ / 01_IDT / graph.xml Swift/Py 分锁；不借此刀对齐曝光格）.
    assert swift_odt == README_FILES_ROW_ODT
    assert py_odt_row == README_FILES_ROW_ODT
    assert swift_odt == py_odt_row
    assert README_FILES_ODT_TO == "709 预览（BT.709 OETF，不是 ACES OT）"
    ...
    # ㉗–㊺ locked strings 一字不动（含 M1 是串行节点图（IDT → 曝光 → 白平衡 → ODT）， / 不是通用节点编辑器 / 任何精度声明前须有灰卡样张。 / 709 预览（BT.709 OETF，不是 ACES OT） / 白平衡 DCTL（精确 3×3） / 白平衡 ASC CDL 校色器 / 白平衡查找表（Bradford CAT，ACEScct 封装） / 01_IDT Role 分锁 / 本说明 / graph.xml 机器可读分锁 / | 文件 | 作用 | / 调色页，串行节点图： / 同一图的 Graphviz）.
    assert README_FILES_ODT_TO == "709 预览（BT.709 OETF，不是 ACES OT）"
    assert README_FILES_ROW_ODT == (
        "| `04_ODT_Rec709.cube` | 709 预览（BT.709 OETF，不是 ACES OT） |"
    )
    assert _files_odt_row(readme) == README_FILES_ROW_ODT
    assert _files_odt_row(readme_fn) == README_FILES_ROW_ODT
    assert _files_odt_row(py_readme) == README_FILES_ROW_ODT
    assert README_FILES_ODT_FROM not in swift_files
    assert README_FILES_ODT_FROM not in py_files
    assert README_FILES_WB_DCTL_TO == "白平衡 DCTL（精确 3×3）"
    assert README_FILES_ROW_WB_DCTL == "| `03_WB.dctl` | 白平衡 DCTL（精确 3×3） |"
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in swift_trail
    assert "达芬奇已验证" not in py_trail
    assert "达芬奇已验证" not in files
    assert "达芬奇已验证" not in README_TRAIL_FOLLOWING
    ...
    _assert_chengpian_not_a_deliverable_claim(README_TRAIL_FOLLOWING)


def test_readme_apply_wb_plain_chinese(tmp_path: Path):
    """README Apply ㊼: 白平衡行人话. Swift↔Py 该行一致. ㉗–㊻ + 邻行 IDT/ODT / TITLE / XML / 色管 frozen."""
    assert README_APPLY_WB_LINE == (
        "- 应用 **白平衡**（节点 3：LUT `03_WB.cube`，**或** DCTL `03_WB.dctl`，"
        "**或** 把 `03_WB.cdl` 导入校色器）。"
    )
    assert README_APPLY_WB_FROM == (
        "- Apply **WB** (node 3: LUT `03_WB.cube`, **or** DCTL `03_WB.dctl`, "
        "**or** import `03_WB.cdl` onto a Color Corrector)."
    )
    assert README_APPLY_WB_BANNED == ("Apply **WB**", "Color Corrector")
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    wb_line = _apply_wb_line(readme)

    assert wb_line == README_APPLY_WB_LINE
    assert README_APPLY_WB_FROM not in apply
    assert README_APPLY_WB_FROM not in readme
    for token in README_APPLY_WB_BANNED:
        assert token not in wb_line, token

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_line = _apply_wb_line(generated)
    assert generated_line == wb_line
    assert generated_line == README_APPLY_WB_LINE
    assert README_APPLY_WB_FROM not in generated
    for token in README_APPLY_WB_BANNED:
        assert token not in generated_line, token
    ...
    assert README_APPLY_WB_FROM not in swift_apply
    assert README_APPLY_WB_FROM not in py_apply
    assert README_APPLY_WB_FROM not in readme_fn
    assert README_APPLY_WB_FROM not in py_readme
    for token in README_APPLY_WB_BANNED:
        assert token not in swift_wb, token
        assert token not in py_wb, token

    # Apply 邻行冻：IDT 见㊽（分锁）. Exposure 见㊿. ODT 见㊾.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert "- 应用 **曝光**" in py_apply
    ...
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in swift_wb
    assert "达芬奇已验证" not in py_wb
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(swift_wb)
    _assert_chengpian_not_a_deliverable_claim(py_wb)
    _assert_chengpian_not_a_deliverable_claim(README_APPLY_WB_LINE)


def test_readme_apply_idt_plain_chinese(tmp_path: Path):
    """README Apply ㊽: IDT 行人话. Swift/Py 分锁不抹平. ㉗–㊼ + 邻行 Exposure/WB/ODT / TITLE / XML / 色管 frozen."""
    assert README_APPLY_IDT_SWIFT == (
        "- 应用 **IDT**（节点 1：LUT `01_IDT_*.cube`，或 ACES IDT / CST 相机 → ACEScct）。"
    )
    assert README_APPLY_IDT_PY == (
        "- 应用 **IDT**（节点 1：LUT `01_IDT_*.cube`，或 CST 相机 → ACEScct，ACES 工作流）。"
    )
    assert README_APPLY_IDT_SWIFT_FROM == (
        "- Apply **IDT** (node 1: LUT `01_IDT_*.cube`, or ACES IDT / CST camera → ACEScct)."
    )
    assert README_APPLY_IDT_PY_FROM == (
        "- Apply **IDT** (node 1: LUT `01_IDT_*.cube`, or CST camera → ACEScct, ACES workflow)."
    )
    assert README_APPLY_IDT_BANNED == "Apply **IDT**"
    assert README_APPLY_IDT_SWIFT != README_APPLY_IDT_PY
    assert "ACES IDT" in README_APPLY_IDT_SWIFT
    assert "ACES 工作流" in README_APPLY_IDT_PY
    assert "ACES 工作流" not in README_APPLY_IDT_SWIFT
    assert "ACES IDT" not in README_APPLY_IDT_PY
    assert README_APPLY_IDT_BANNED in README_APPLY_IDT_SWIFT_FROM
    assert README_APPLY_IDT_BANNED in README_APPLY_IDT_PY_FROM
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    idt_line = _apply_idt_line(readme)

    assert idt_line == README_APPLY_IDT_PY
    assert README_APPLY_IDT_SWIFT not in apply
    assert README_APPLY_IDT_SWIFT not in readme
    assert README_APPLY_IDT_SWIFT_FROM not in apply
    assert README_APPLY_IDT_PY_FROM not in apply
    assert README_APPLY_IDT_SWIFT_FROM not in readme
    assert README_APPLY_IDT_PY_FROM not in readme
    assert README_APPLY_IDT_BANNED not in idt_line

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_line = _apply_idt_line(generated)
    assert generated_line == idt_line
    assert generated_line == README_APPLY_IDT_PY
    assert README_APPLY_IDT_SWIFT not in generated
    assert README_APPLY_IDT_SWIFT_FROM not in generated
    assert README_APPLY_IDT_PY_FROM not in generated
    assert README_APPLY_IDT_BANNED not in generated_line
    ...
    assert README_APPLY_IDT_PY not in swift_apply
    assert README_APPLY_IDT_SWIFT not in py_apply
    assert "ACES IDT" in swift_idt
    assert "ACES 工作流" in py_idt
    assert "ACES 工作流" not in swift_idt
    assert "ACES IDT" not in py_idt
    assert README_APPLY_IDT_SWIFT_FROM not in swift_apply
    assert README_APPLY_IDT_PY_FROM not in py_apply
    assert README_APPLY_IDT_SWIFT_FROM not in readme_fn
    assert README_APPLY_IDT_PY_FROM not in py_readme
    assert README_APPLY_IDT_BANNED not in swift_idt
    assert README_APPLY_IDT_BANNED not in py_idt

    # Apply 邻行冻：Exposure 见㊿. WB / ODT.
    assert "- 应用 **曝光**" in py_apply
    assert README_APPLY_EXP_LINE in py_apply
    assert swift_wb == README_APPLY_WB_LINE
    assert py_wb == README_APPLY_WB_LINE
...
def test_readme_apply_odt_line_plain_chinese(tmp_path: Path):
    ...
    # Apply 邻行冻：IDT 见㊽（分锁）. Exposure 见㊿. WB 见㊼.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert "- 应用 **曝光**" in py_apply
...
def test_readme_apply_exposure_plain_chinese(tmp_path: Path):
    """README Apply ㊿: Py 曝光行人话. Swift 无对行. ㉗–㊾ + 邻行 IDT/WB/ODT / TITLE / XML / 色管 frozen."""
    assert README_APPLY_EXP_LINE == (
        "- 应用 **曝光**（节点 2：LUT `02_Exposure.cube` 或 DCTL `02_Exposure.dctl`）。"
        "0 档或旁路 = 单位阵。"
    )
    assert README_APPLY_EXP_FROM == (
        "- Apply **Exposure** (node 2: LUT `02_Exposure.cube` or DCTL `02_Exposure.dctl`). "
        "Zero stops or bypass = identity."
    )
    assert README_APPLY_EXP_BANNED == ("Apply **Exposure**", "Zero stops or bypass")
    ...
    apply = _apply_section(readme)
    files = _files_section(readme)
    exp_line = _apply_exposure_line(readme)

    assert exp_line == README_APPLY_EXP_LINE
    assert README_APPLY_EXP_FROM not in apply
    assert README_APPLY_EXP_FROM not in readme
    for token in README_APPLY_EXP_BANNED:
        assert token not in exp_line, token

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_line = _apply_exposure_line(generated)
    assert generated_line == exp_line
    assert generated_line == README_APPLY_EXP_LINE
    assert README_APPLY_EXP_FROM not in generated
    for token in README_APPLY_EXP_BANNED:
        assert token not in generated_line, token
    ...
    # 验法㊿-1: Py one TO 一字不差. Swift 无对行，不假造.
    assert py_exp == README_APPLY_EXP_LINE
    assert README_APPLY_EXP_FROM not in py_apply
    assert README_APPLY_EXP_FROM not in py_readme
    for token in README_APPLY_EXP_BANNED:
        assert token not in py_exp, token
    swift_exp_lines = [
        ln.strip()
        for ln in swift_apply.splitlines()
        if ln.strip().startswith("- 应用 **曝光**") or ln.strip().startswith("- Apply **Exposure**")
    ]
    assert swift_exp_lines == []
    with pytest.raises(AssertionError, match="Apply Exposure line missing"):
        _apply_exposure_line(readme_fn)
    assert README_APPLY_EXP_LINE not in swift_apply
    assert README_APPLY_EXP_FROM not in swift_apply
    assert README_APPLY_EXP_FROM not in readme_fn
    assert "- 应用 **曝光**" not in swift_apply
    assert "- Apply **Exposure**" not in swift_apply

    # Apply 邻行冻：IDT 见㊽（分锁）. WB 见㊼. ODT 见㊾.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert (
...
def test_readme_bypass_wb_swift_plain_chinese(tmp_path: Path):
    """README 51: Swift 旁路白平衡短句. Py 长句见 52. ㉗–㊿ + 邻行 Apply / TITLE / XML / 色管 frozen."""
    ...
    assert py_bypass == README_BYPASS_WB_PY_TO
    ...
    assert generated_bypass == README_BYPASS_WB_PY_TO
    ...
    # 验法51-1: Swift one TO 一字不差. Py 长句见 52，不假造、不拉平.
    ...
    assert py_src_bypass == README_BYPASS_WB_PY_TO
    ...
    assert README_BYPASS_WB_SWIFT_TO != README_BYPASS_WB_PY_TO
    ...
    assert "旁路曝光：" in py_apply
    assert "Bypass Exposure" in py_src_bypass
    assert "Bypass WB" in py_src_bypass
...
def test_readme_bypass_exposure_wb_py_plain_chinese(tmp_path: Path):
    """README 52: Py 旁路曝光/白平衡长句. Swift 旁路已锁另冻. ㉗–51 + 邻行 Apply / TITLE / XML / 色管 frozen."""
    assert README_BYPASS_WB_PY_TO == (
        "旁路曝光：关掉节点 2（或勾 DCTL **Bypass Exposure**，或停在 0 档）。"
        "旁路白平衡：关掉节点 3（或勾 DCTL **Bypass WB**，或跳过 CDL/LUT）。"
        "剩余图：**IDT →（可选曝光）→ ACEScct → 可选 Rec.709 ODT**。"
    )
    assert README_BYPASS_WB_PY_FROM == (
        "To bypass Exposure: disable node 2 (or tick DCTL **Bypass Exposure**, or leave stops at 0). "
        "To bypass WB: disable node 3 (or tick DCTL **Bypass WB**, or skip the CDL/LUT). "
        "The remaining graph is **IDT → (optional Exposure) → ACEScct → optional Rec.709 ODT**."
    )
    assert README_BYPASS_WB_PY_BANNED == (
        "To bypass Exposure",
        "The remaining graph is",
        "剩余图是",
        "档位留 0",
    )
    assert "Bypass Exposure" in README_BYPASS_WB_PY_TO
    assert "Bypass WB" in README_BYPASS_WB_PY_TO
    assert "剩余图：" in README_BYPASS_WB_PY_TO
    assert "停在 0 档" in README_BYPASS_WB_PY_TO
    assert README_BYPASS_WB_SWIFT_TO == (
        "旁路白平衡：关掉节点 2（或勾 DCTL **Bypass WB**，或跳过 CDL/LUT）。"
        "剩余图：**IDT → ACEScct**，不烘焙。"
    )
    ...
    assert py_bypass == README_BYPASS_WB_PY_TO
    assert README_BYPASS_WB_PY_FROM not in apply
    assert README_BYPASS_WB_PY_FROM not in readme
    for token in README_BYPASS_WB_PY_BANNED:
        assert token not in py_bypass, token
    assert README_BYPASS_WB_SWIFT_TO not in apply
    assert README_BYPASS_WB_SWIFT_TO not in readme
    assert README_BYPASS_WB_SWIFT_FROM not in apply
    assert README_BYPASS_WB_SWIFT_FROM not in readme
    ...
    generated_bypass = _bypass_exposure_wb_py_line(generated)
    assert generated_bypass == README_BYPASS_WB_PY_TO
    assert README_BYPASS_WB_PY_FROM not in generated
    for token in README_BYPASS_WB_PY_BANNED:
        assert token not in generated_bypass, token
    assert README_BYPASS_WB_SWIFT_TO not in generated
    assert README_BYPASS_WB_SWIFT_FROM not in generated
    ...
    # 验法52-1: Py one TO 一字不差. Swift 旁路已锁另冻，不假造、不拉平.
    assert py_src_bypass == README_BYPASS_WB_PY_TO
    assert py_src_bypass == py_bypass
    assert "Bypass Exposure" in py_src_bypass
    assert "Bypass WB" in py_src_bypass
    assert "剩余图：" in py_src_bypass
    assert "停在 0 档" in py_src_bypass
    assert README_BYPASS_WB_PY_FROM not in py_apply
    assert README_BYPASS_WB_PY_FROM not in py_readme
    assert README_BYPASS_WB_PY_FROM not in py
    for token in README_BYPASS_WB_PY_BANNED:
        assert token not in py_src_bypass, token
    assert swift_bypass == README_BYPASS_WB_SWIFT_TO
    assert "Bypass WB" in swift_bypass
    assert README_BYPASS_WB_SWIFT_FROM not in swift_apply
    assert README_BYPASS_WB_SWIFT_FROM not in readme_fn
    ...
    assert README_BYPASS_WB_SWIFT_TO not in py_apply
    assert README_BYPASS_WB_SWIFT_TO not in py_readme
    assert README_BYPASS_WB_PY_TO not in swift_apply
    assert README_BYPASS_WB_PY_TO not in readme_fn
    assert README_BYPASS_WB_SWIFT_TO != README_BYPASS_WB_PY_TO
    assert swift_bypass != py_src_bypass
    with pytest.raises(AssertionError, match="Swift bypass WB line missing"):
        _bypass_wb_swift_line(py_readme)
    ...
    # Apply 邻行冻：IDT 见㊽（分锁）. Exposure 见㊿. WB 见㊼. ODT 见㊾. Swift 旁路见 51.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert (
    ...
    assert README_APPLY_EXP_LINE in py_apply
    assert README_APPLY_EXP_LINE not in swift_apply
    assert README_BYPASS_WB_SWIFT_TO in swift_apply
    assert README_BYPASS_WB_SWIFT_TO not in py_apply

    # ㉗–51 locked strings 一字不动.
    assert README_APPLY_WB_LINE in swift_apply
    assert README_APPLY_WB_LINE in py_apply
    assert README_APPLY_IDT_SWIFT in swift_apply
    assert README_APPLY_IDT_PY in py_apply
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in py_src_bypass
    assert "达芬奇已验证" not in files
...
def test_readme_deliverable_warn_py_plain_chinese(tmp_path: Path):
    """README 53: Py Rec.709 交付警告. Swift 无此句勿动. ㉗–52 + 邻行 Apply / TITLE / XML / 色管 frozen."""
    assert README_DELIVERABLE_WARN_PY_TO == (
        "不要把单个 Rec.709 文件当作唯一交付物。Rec.709 仅预览。"
    )
    assert README_DELIVERABLE_WARN_PY_FROM == (
        "Do not use a single Rec.709 file as the only deliverable. Rec.709 is preview only."
    )
    assert README_DELIVERABLE_WARN_PY_BANNED == (
        "Do not use a single Rec.709",
        "as the only deliverable",
        "the only deliverable",
        "Rec.709 is preview only",
    )
    assert "仅预览" in README_DELIVERABLE_WARN_PY_TO
    assert "唯一交付物" in README_DELIVERABLE_WARN_PY_TO
    assert README_BYPASS_WB_PY_TO == (
        "旁路曝光：关掉节点 2（或勾 DCTL **Bypass Exposure**，或停在 0 档）。"
        "旁路白平衡：关掉节点 3（或勾 DCTL **Bypass WB**，或跳过 CDL/LUT）。"
        "剩余图：**IDT →（可选曝光）→ ACEScct → 可选 Rec.709 ODT**。"
    ...
    files = _files_section(readme)
    py_bypass = _bypass_exposure_wb_py_line(readme)
    py_warn = _deliverable_warn_py_line(readme)

    assert py_warn == README_DELIVERABLE_WARN_PY_TO
    assert README_DELIVERABLE_WARN_PY_FROM not in apply
    assert README_DELIVERABLE_WARN_PY_FROM not in readme
    for token in README_DELIVERABLE_WARN_PY_BANNED:
        assert token not in py_warn, token
    assert py_bypass == README_BYPASS_WB_PY_TO
    assert README_BYPASS_WB_PY_FROM not in apply
    assert README_BYPASS_WB_PY_FROM not in readme
    assert README_BYPASS_WB_SWIFT_TO not in apply
    ...
    assert README_BYPASS_WB_SWIFT_FROM not in apply
    assert README_BYPASS_WB_SWIFT_FROM not in readme
    bypass_idx = apply.find(README_BYPASS_WB_PY_TO)
    warn_idx = apply.find(README_DELIVERABLE_WARN_PY_TO)
    files_idx = apply.find("## Files")
    assert 0 <= bypass_idx < warn_idx < files_idx

    generated = format_readme(["arri_logc4_awg4"], 3200.0, 0.25, True)
    generated_warn = _deliverable_warn_py_line(generated)
    generated_bypass = _bypass_exposure_wb_py_line(generated)
    assert generated_warn == README_DELIVERABLE_WARN_PY_TO
    assert README_DELIVERABLE_WARN_PY_FROM not in generated
    for token in README_DELIVERABLE_WARN_PY_BANNED:
        assert token not in generated_warn, token
    assert generated_bypass == README_BYPASS_WB_PY_TO
    assert README_BYPASS_WB_SWIFT_TO not in generated
    assert README_BYPASS_WB_SWIFT_FROM not in generated
    ...
    swift_bypass = _bypass_wb_swift_line(readme_fn)
    py_src_bypass = _bypass_exposure_wb_py_line(py_readme)
    py_src_warn = _deliverable_warn_py_line(py_readme)

    # 验法53-1: Py one TO 一字不差. Swift 无此句，不假造、不拉平.
    assert py_src_warn == README_DELIVERABLE_WARN_PY_TO
    assert py_src_warn == py_warn
    assert "仅预览" in py_src_warn
    assert "唯一交付物" in py_src_warn
    assert README_DELIVERABLE_WARN_PY_FROM not in py_apply
    assert README_DELIVERABLE_WARN_PY_FROM not in py_readme
    assert README_DELIVERABLE_WARN_PY_FROM not in py
    for token in README_DELIVERABLE_WARN_PY_BANNED:
        assert token not in py_src_warn, token
    assert README_DELIVERABLE_WARN_PY_TO not in swift_apply
    assert README_DELIVERABLE_WARN_PY_TO not in readme_fn
    assert README_DELIVERABLE_WARN_PY_FROM not in swift_apply
    assert README_DELIVERABLE_WARN_PY_FROM not in readme_fn
    assert README_DELIVERABLE_WARN_PY_FROM not in swift
    with pytest.raises(AssertionError, match="Py Rec.709 deliverable warning missing"):
        _deliverable_warn_py_line(readme_fn)
    assert py_src_bypass == README_BYPASS_WB_PY_TO
    assert py_src_bypass == py_bypass
    assert "剩余图：" in py_src_bypass
    assert "Bypass Exposure" in py_src_bypass
    ...
    # Apply 邻行冻：IDT 见㊽（分锁）. Exposure 见㊿. WB 见㊼. ODT 见㊾. Swift 旁路见 51. Py 旁路见 52.
    assert "- 应用 **IDT**" in apply
    assert "- 应用 **IDT**" in swift_apply
    assert "- 应用 **IDT**" in py_apply
    assert (
    ...
    assert README_BYPASS_WB_SWIFT_TO in swift_apply
    assert README_BYPASS_WB_SWIFT_TO not in py_apply
    assert README_BYPASS_WB_PY_TO in py_apply
    assert README_BYPASS_WB_PY_TO not in swift_apply
    assert "## How to bypass Exposure / WB in Resolve" in py_readme
    assert "## How to bypass WB in Resolve" in readme_fn
    assert "## How to bypass Exposure / WB in Resolve" not in readme_fn
    assert README_FILES_HEADER_TO in files
    assert README_FILES_ROW_XML_PY in files
    assert README_FILES_GRAPH_DOT_ROW in files
    assert README_FILES_ROW_README in files
    assert README_FILES_HEADER_FROM not in files
    assert README_FILES_HEADER_TO in py_readme
    assert README_FILES_ROW_XML_PY in py_readme

    # ㉗–52 locked strings 一字不动.
    assert README_APPLY_WB_LINE in swift_apply
    assert README_APPLY_WB_LINE in py_apply
    assert README_APPLY_IDT_SWIFT in swift_apply
    assert README_APPLY_IDT_PY in py_apply
    ...
    assert "white_balance_matrix" in py
    assert "def apply_exposure" in (root / "color/exposure.py").read_text(encoding="utf-8")
    assert "达芬奇已验证" not in py_src_warn
    assert "达芬奇已验证" not in py_src_bypass
    assert "达芬奇已验证" not in files
    _assert_chengpian_not_a_deliverable_claim(py_src_warn)
    _assert_chengpian_not_a_deliverable_claim(README_DELIVERABLE_WARN_PY_TO)
    _assert_chengpian_not_a_deliverable_claim(py_src_bypass)
    _assert_chengpian_not_a_deliverable_claim(README_BYPASS_WB_PY_TO)
    _assert_chengpian_not_a_deliverable_claim(README_BYPASS_WB_SWIFT_TO)
