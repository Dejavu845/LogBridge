...
- **Primary button:** **处理已锁定片段** (never 一键还原). Shown only when locked-clip count > 0. Writes an ACES2065-1 AP0 proxy EXR sequence — **整段代理，不是全精度成片.** Not ACEScct. Unlocked clips are skipped. **Apply graph** is the same batch, not a second button.
