...
- **Block process:** a clip is processable only when its paired IDT is locked. **处理已锁定片段** appears only when locked-clip count > 0 and **writes one ACES2065-1 EXR per locked clip**. Pending / unlocked stay in the list with **先选择 Log 与色域** or **先选择成对 IDT** and produce **no** output file. Status: **N 条已锁定 / M 条待选**. **N 条已处理** is files written (or attempted with a per-clip error), not a preview refresh. A mixed bin is allowed — do not require every clip locked. No silent IDT. Never guess.
...
- **Primary button:** **处理已锁定片段** (never 一键还原). Shown only when locked-clip count > 0. Writes ACEScct / ACES2065-1 EXR for locked clips. Unlocked clips are skipped. **Apply graph** is the same batch, not a second button.
...
Export default: **ACEScct** timeline / **ACES2065-1** (**处理已锁定片段** writes ACES2065-1 EXR; **导出 ACEScct / EXR** in **高级** is the Resolve graph). Rec.709 ODT is an optional preview node (off by default). Rec.2100 HLG/PQ are optional ACES/BT.2100 OT nodes (unverified). Locked clips write even when other clips are still pending. Implemented (unverified).
...
**处理已锁定片段** writes one ACES2065-1 EXR per locked clip (same decode path; movies = first frame). That counter is files on disk, not the 8-bit preview. The Resolve graph in **高级** is a separate LUT/XML export (P1), not a second process button.
