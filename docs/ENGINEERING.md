...
- **Do not add cameras.** D-Log M stays a stub (`color/stubs.py`, `FutureIDTs.swift`). Cycle 21 locks `FutureIDTs.dLogMIsSupported()` as false. Cycle 22: `picker_pairs()` must not list `dji_dlog_m`. Cycle 23: Swift `IDT.pickerPairs` uses `implemented`, not `allCases`. Do not invent a transfer.
