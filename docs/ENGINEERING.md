...
- **Do not add cameras.** D-Log M stays a stub (`color/stubs.py`, `FutureIDTs.swift`). Cycle 21 locks `FutureIDTs.dLogMIsSupported()` as false. Cycle 22–24: Python `picker_pairs` and Swift `IDT.pickerPairs` stay stub-free and ID-aligned. Do not invent a transfer.
